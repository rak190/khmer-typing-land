...
// Targets are Khmer characters or common multi-grapheme outputs produced by NiDA mapping.
...
const COMMON_COMBOS = ["ុំ","ុះ","េះ","ោះ","ៀ","ឿ","ាំ"]; // NiDA outputs like "េះ", "ោះ", etc.
const PUNCT = ["។","៕","៖","ៈ","?","!","(",")",",",".","/"];
const KHMER_DIGITS = ["០","១","២","៣","៤","៥","៦","៧","៨","៩"];

// Helper: chunk an array into N roughly-equal chunks
...
// Build 9 stages from a base list + optionally add extras per later stages
function buildStagesFrom(namePrefix, baseList, extrasByStage = {}){
...
export function buildWorlds(){
  // World unlock stars (feel-good pacing)
  // Total max stars = 81 * 3 = 243
  // Unlock gates: W1 0, W2 12, W3 24, ..., W9 96 (easy to reach gradually)
  const unlock = [0,12,24,36,48,60,72,84,96];

  // WORLD 1: Basic consonants (small sets)
  const w1Stages = buildStagesFrom(
    "Consonants (Basics)",
    CONSONANTS.slice(0, 27), // first 27
    { 7:[" "], 8:[" "], 9:[" "] }
  );
  ...
  const w3Stages = buildStagesFrom(
    "Vowels & Signs",
    VOWELS_SIGNS,
    { 1:["ក","គ"], 2:["ត","ន"], 3:["ប","ម"], 9:[" "]} // mix consonants for syllable feel
  );
  ...
  // WORLD 5: Common combos (NiDA outputs)
  ...
  // WORLD 6: Mixed “syllable-like” targets (single grapheme still)
  ...
  // WORLD 7: Sentences punctuation focus (still target chars)
  ...
  // WORLD 8: Numbers + mixed (typing endurance)
  ...
  // WORLD 9: Mastery (everything we used)
  const MASTER_POOL = [...new Set([
